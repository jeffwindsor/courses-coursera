...
(define (mupllist->racketlist ms)
  (if (aunit? (apair-e2 ms)) 
      (cons (apair-e1 ms) null)
  ...
  (cond [(var? e) 
        ...
        [(fst? e) 
         (let ([v (eval-under-env (fst-e e) env)])
           (if (apair? v) 
               (apair-e1 v) 
               (error "MUPL first applied to a non-pair")))]
        [(snd? e) 
         (let ([v (eval-under-env (snd-e e) env)])
           (if (apair? v) 
               (apair-e2 v) 
               (error "MUPL second applied to a non-pair")))]
        [(mlet? e)
        ...
        [(call? e)
         (let ([c (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? c) 
               (let* ([clo-env (closure-env c)]
                      [clo-fun (closure-fun c)]
                      [new-env (cons (cons (fun-formal clo-fun) arg) clo-env)])
                 (if (eq? #f (fun-nameopt clo-fun))
                     (eval-under-env (fun-body clo-fun) new-env)
                     (eval-under-env (fun-body clo-fun) (cons (cons (fun-nameopt clo-fun) c) new-env))))
               (error "MUPL call on non-closure of function")))]
...
(define (mupl-map f)
  (fun "mupl-map-fun" "xs" 
       (ifaunit (snd (var "xs")) 
               (apair (call f (fst (var "xs"))) 
                      (aunit))
               (apair (call f (fst (var "xs"))) 
                      (call (var "mupl-map-fun") (snd (var "xs"))))
                      )))

(define mupl-mapAddN 
        (fun #f "i"
             (mupl-map (fun #f "x" (add (var "x") (var "i"))))))
