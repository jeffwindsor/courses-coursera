...
class Point < GeometryValue
  ...
  def in_between(v, end1, end2)
    this = (end1 - GeometryExpression::Epsilon <= v && v <= end2 + GeometryExpression::Epsilon)
    that = (end2 - GeometryExpression::Epsilon <= v && v <= end1 + GeometryExpression::Epsilon)
    this || that
  end
...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    x_close = real_close(x1,x2)
    if x_close && real_close(y1,y2)
      Point.new(x1,y1)
    elsif (x_close && (y1 > y2)) ||  (x1 > x2)
...
class Var < GeometryExpression
  ...
  def eval_prog env
    found = env.detect { |pair| pair[0] == s }
    #puts "var found"
    #puts found[0]
    #puts found[1]
    found[1]
