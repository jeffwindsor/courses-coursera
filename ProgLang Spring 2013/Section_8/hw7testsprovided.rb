...
if not ((d2.is_a? Point)and d2.x == ONE and d2.y == TWO) 
	puts "LineSegment preprocess_prog should convert to a Point, if ends of segment are real_close"
end

d = d.preprocess_prog
if not (d.x1 == -THREE and d.y1 == -FOUR and d.x2 == ONE and d.y2 == TWO)
	puts "LineSegment preprocess_prog should make x1 and y1 on the left of x2 and y2"
end
...
#Var#eval_prog: Var.new("y") should evaluate to Point.new(0.0,0.0)
#   given env==[["x", NoPoints.new], ["y", Point.new(0.0,0.0)]]
#   (actual: NoPoints.new) [incorrect answer]
v = Var.new("y")
env = [["x", NoPoints.new], ["y", Point.new(0.0,0.0)]]
#env.each {|pair| puts pair[0] + " : " + pair[1] }
#found = env.select { |pair| pair[0] == "y" }
#puts found[0] + " : " + found[1]
v1 = v.eval_prog(env)
if not ((v1.is_a? Point) and v1.x == ZERO and v1.y == ZERO)
  puts "Var eval_prog is not working properly"
  puts "  Result : " && (v1)
  puts "  Is Point : " && (v1.is_a? Point)
  puts "  X : " && (v1.x)
  puts "  Y : " && (v1.y)
end

#Var#eval_prog: Var.new("y") should evaluate to Point.new(0.0,0.0)
#   given env==[["x", NoPoints.new], ["y", Point.new(0.0,0.0)], ["x", Point.new(3.0,2.0)]]
#   (actual: NoPoints.new) [incorrect answer]
v = Var.new("y")
env = [["x", NoPoints.new], ["y", Point.new(0.0,0.0)], ["x", Point.new(3.0,2.0)]]
#env.each {|pair| puts pair[0] + " : " + pair[1] }
#found = env.select { |pair| pair[0] == "y" }
#puts found[0] + " : " + found[1]
v1 = v.eval_prog(env)
...
#Let#eval_prog: Let.new("x",NoPoints.new,Let.new("y",Point.new(0.0,0.0),Let.new("z",Point.new(1.0,1.0),Var.new("x"))))
#   should evaluate to NoPoints.new
#   given env==[]
#   (actual: Point.new(1.0,1.0)) [incorrect answer]
l = Let.new("x",NoPoints.new, Let.new("y",Point.new(0.0,0.0), Let.new("z",Point.new(1.0,1.0), Var.new("x"))))
l1 = l.eval_prog([])
if not (l1.is_a? NoPoints)
  puts "Let eval_prog is not working properly"
  puts l1
end