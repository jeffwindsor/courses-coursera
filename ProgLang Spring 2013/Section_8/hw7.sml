...
fun preprocess_prog(e) = 
	case e of 
		 LineSegment(x1,y1,x2,y2) => 
		 	let
		 		val x_is_close = real_close(x1, x2)
		 		val y_is_close = real_close(y1, y2)
		 		val x_swapped = (x1 > x2)
		 		val y_swapped = (y1 > y2)
		 	in
		 		case (x_is_close,y_is_close,x_swapped,y_swapped) of
		 			  (true,true,_,_) 		=> Point(x1,y2)
		 			| (true,false,_,true)	=> LineSegment(x2,y2,x1,y1)
		 			| (false,_,true,_)			=> LineSegment(x2,y2,x1,y1)
		 			| (_,_,_,_) 			=> e
		 	end 
